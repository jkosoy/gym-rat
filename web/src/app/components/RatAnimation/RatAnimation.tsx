...
export function RatAnimation({}: PropsWithoutRef<RatAnimationProps>) {
  ...
  const videoEls = (() => {
    return ["jump", "flex", "laugh-intro", "laugh-loop"].map(name => {
      ...
      return (
        <video key={`video_${name}`}  className={className} onEnded={handleVideoEnded} width="1920" height="1920" preload="auto" loop={name === "laugh-loop"} autoPlay={name === state} poster="/transparent.png" muted>
